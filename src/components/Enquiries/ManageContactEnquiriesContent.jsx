...
} from "../../services/contactEnquiryServices";
...
export const ManageContactEnquiriesContent = () => {
  ...
  const filteredContactEnquiries = contactEnquiries.filter((item) => {
  ...
  const currentData = paginateData(
    filteredContactEnquiries,
    currentPage,
    entriesPerPage
  );
  const totalPages = calculateTotalPages(
    filteredContactEnquiries.length,
    entriesPerPage
  );
  ...
  const handleDelete = async (id) => {
    const confirmed = await confirmDelete("contact enquiry");
    if (confirmed) {
      try {
        await deleteContactEnquiry(id);
        setContactEnquiries((prev) => prev.filter((item) => item.id !== id));
        Swal.fire(
          "Deleted!",
          "Contact enquiry has been deleted successfully.",
          "success"
        );
      } catch (error) {
        handleErrors(error);
          ...
          <div className="col-xxl-12">
            <div className="card mt-xxl-n5">
              <div className="card-header">
                <h5 className="mb-sm-2 mt-sm-2">Manage Contact Enquiries</h5>
              </div>
              <div className="card-body manage-amenity-master-card-body">
                ...
                <Pagination
                  ...
                  totalEntries={filteredContactEnquiries.length}
                  entriesPerPage={entriesPerPage}
